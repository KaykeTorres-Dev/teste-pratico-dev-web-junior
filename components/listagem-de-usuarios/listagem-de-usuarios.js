...
.catch(error => {
  ...
  alert('Ocorreu um erro ao carregar lista de usuários. Por favor, tente novamente.');
});
...
function deleteUser(userId) {
  ...
  .then(response => {
    if (!response.ok) {
      throw new Error('Erro ao excluir os usuário!');
    }
    return response.json();
  })
  .then(data => {
    alert('Usuário excluído com sucesso!', data);
    location.reload();
